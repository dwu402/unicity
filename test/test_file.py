# Python file for unicity unit tests
# File implements lu_factorisation
...
def lu_factor(A, pivot=False, display=False):
	"""
	Return LU factors and row swap vector p of square matrix A.
...
def lu_forward_sub(L, b, p=None):
	"""
	Return intermediate vector y from solution of lower triangular equations.
	
    Parameters
    ----------
	L : np.array
		LU factorized matrix, only lower triangular section used
	b : np.array
		RHS vector
	p : np.array
		Row swap vector returned by LU_FACTOR
			
    Returns
    -------
	y : np.array
		Matrix of intermediate vectors (each a column)

    Notes
    -----
	Solve linear system L*y=b where L is lower triangular square matrix
...
def lu_backward_sub(U, y):
	"""
	Return solution vector from solution of upper triangular equations
	
    Parameters
    ----------
	A : np.array
		LU factorized matrix, only upper triangular section used
	y : np.array
		Matrix of intermediate vectors (each a column), 
		derived from forward substitution 
	...
	Notes
    -----
	solves linear system U*x=y where U is upper triangular square matrix.
...
def isSquare (A):
	...
	True
	"""
	B = all (len (row) == len (A) for row in A)
	return B